...
export class MetaCatalogHelperService {
  ...
  public async addProductToCatalog(catalogId: string, product: any) {
    ...
    const response = await this.metaApiHelper.triggerMetaApi(
      'post',
      url,
      product,
    );
  ...
  public async getProduct(productId: string) {
    ...
    const params = {
      fields:
        'id,retailer_id,name,description,price,currency,category,image_url,additional_image_urls,availability,url,sale_price',
    };
    const response = await this.metaApiHelper.triggerMetaApi(
      'get',
      url,
